...
export function stdout(data: Buffer | string) {
  ...
  if (output.stdout.length + dataString.length < buffer.constants.MAX_STRING_LENGTH) {
...
export function stderr(data: Buffer | string) {
  const dataString = data.toString().trim();
  if (output.stderr.length + dataString.length < buffer.constants.MAX_STRING_LENGTH) {
